...
public class InfoAgent extends JFrame implements ActionListener{
	/**
	 * 
	 */
	...
	public InfoAgent()
	{
		...
		try {
			...
			String req = "SELECT Lib_POSTE,Grade_POSTE,Salaire,Date_Occupation FROM carierre,poste "
			...
			/*JPanel paneB1=new JPanel(new GridLayout(1,3,20,20));
			paneB1.add(add);
			paneB1.add(sup);
			paneB1.add(mod);
			pane1.add(paneB1,BorderLayout.WEST);*/
		...
		// tableau des diplomes 
				...
				try {
					conn=login.Connexion.getConnection();
					String req = "SELECT Nom_DIPLOME, Date_Obtention,Specialite_DIPLOME,"
							+"Etablissement,Mention_DIPLOME "
							+ "FROM `diplome` WHERE Id_AGNET ="+agent.getIdAgent();
					PreparedStatement af=conn.prepareStatement(req);
					ResultSet rst =af.executeQuery(req);
				...
				try {
					...
					String req = "SELECT Type_Conge,Date_Debut,Date_Fin FROM conge "
							+ "WHERE Id_AGNET = "+ agent.getIdAgent();
					PreparedStatement af=conn.prepareStatement(req);
					ResultSet rst =af.executeQuery(req);
				...
				c.add(pane);
				//.add(new JButton("Deconnecter"),BorderLayout.SOUTH);
